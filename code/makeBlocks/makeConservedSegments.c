...
// fprintf(stderr, "- merging orthology blocks %d to %d\n", start, terminal);
...
//	fprintf(stderr, "- skipping orthology block %d\n", start);
		return;
	}
...
int main(int argc, char* argv[]) {
	...
	get_spename(argv[1]);
	//get_minlen(argv[1]);
	//MINLEN_OUTGROUP = ((int)MINLEN*0.2);
	//MINLEN_OUTGROUP = MINLEN;
//printf("MINLEN_OUTGROUP=%d\n", MINLEN_OUTGROUP);
	rs = ref_spe_idx();
	...
	// get signed permutation in each species.
	// chromosomes are separated by 0.
	// e.g. 0 1 2 3 0 4 5 6 0 7 8 9 ...
	...
	for (s = blkhead; s != NULL; s = s->next) {
		...
		for (i = 0; i < Spesz; i++) {
			for (p = s->speseg[i]; p != NULL; p = p->next) {
				// check outgroup block size
				//if (Spetag[i] > 1 && (p->end - p->beg) <= MINLEN_OUTGROUP) continue; 
